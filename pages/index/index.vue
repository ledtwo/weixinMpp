  ...
  <view>
    ...
    <view class="bigbox">
      <view class="firstbox dis-jasc">
        <view
          class="firsone dis-fdjacc"
          v-for="(item, index) in somemun"
          :key="index"
        >
          <view>{{ item.til }}</view>
    ...
    <view class="lastlist" v-show="current == 0" @click="longtap = -1">
      <view
        class="listone dis-alicen"
        v-for="(item, index) in lastlists"
        :key="index"
        @longpress="showalert(index)"
      >
        <view class="oneimg">
    ...
    >
      <view class="showpop">
        ...
        <view class="ercode" v-show="showone">
          ...
          <view class="codtwo"
            >2.再用微信“扫一扫”对着另一台设备拍摄的二维码扫码</view
          >
          <view class="codimg">
          ...
          <view class="codlas"
            >1.扫码后，扫码微信号将变成机器人（不影响微信号正常使用）</view
          >
        </view>
      ...
      <view class="showpop">
        ...
        <view class="retuwater" v-if="usersnum == 2">
          ...
          <view class="retils dis-alicen">
            <u-icon name="warning-fill" color="#FF7900" size="32"></u-icon>
            <text
              >温馨提示：反水比例按照10000流水基数，百分之几由自己设定，请在上方输入并保存！</text
            >
          </view>
